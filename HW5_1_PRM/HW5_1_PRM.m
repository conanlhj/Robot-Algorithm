...
%% Define Goal
q_goal = [0 -5 0 0 pi/4 0];
Goal = collisionBox(1, 1, 5);
Goal.Pose = trvec2tform(q_goal(1:3)) * eul2tform(q_goal(4:6));


%% Define Obj
q_init = [0, 5, 0, 0, pi/2, 0];
Obj = collisionBox(1, 1, 5);
Obj.Pose = trvec2tform(q_init(1:3)) * eul2tform(q_init(4:6));

%% Roadmap Construction
G = f_Roadmap_Construction(N, K, Obs, false);


%% Query_Algorithm
G = f_Query_Algorithm(q_init, q_goal, K, G, Obs);


%% Path finding
P = shortestpath(G, findnode(G, "q_init"), findnode(G, "q_goal"));
clf;
f_draw(Obs, Obj, Goal, true);
plotconfig = plot(G, 'XData', G.Nodes.Point(:, 1), ...
    'YData', G.Nodes.Point(:, 2), 'ZData', G.Nodes.Point(:, 3), 'MarkerSize', 0.01, 'EdgeAlpha', 0.08);
highlight(plotconfig, P, 'NodeColor','g','EdgeColor','g', 'MarkerSize', 4, 'LineWidth', 2);
